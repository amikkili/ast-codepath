...
// FIX: Fetches real plan from DB on every load — session JWT can be stale
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // ── FIX: Always fetch real plan from DB ──────────────────────────
  ...
  // Use real DB plan — not cached JWT session plan
        ...
        {plan === 'FREE' && (
          <div className="bg-[#1e1e2a] border border-[#534AB7]/40 rounded-xl p-4 mb-8 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-[#5a6278] mt-0.5">You are on the Free plan. Basic unlocks all videos + unlimited AI agent for $12/month.</p>
            </div>
            <Link href="/upgrade" className="bg-[#534AB7] text-[#EEEDFE] text-xs font-medium px-4 py-2 rounded-lg hover:opacity-90 flex-shrink-0">
              Upgrade now →
            </Link>
          </div>
        )}
        ...
        {plan === 'BASIC' && (
          <div className="bg-[#1a2a1e] border border-[#1D9E75]/30 rounded-xl p-4 mb-8 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-[#5a6278] mt-0.5">You are on Basic. Pro adds weekly live classes, 1:1 mentor hours and job prep for $39/month.</p>
            </div>
            <Link href="/upgrade" className="bg-[#1D9E75] text-white text-xs font-medium px-4 py-2 rounded-lg hover:opacity-90 flex-shrink-0">
              Upgrade to Pro →
            </Link>
          </div>
        )}
        ...
        {plan === 'PRO' && (
          <div className="bg-[#1e2a1e] border border-[#1D9E75]/20 rounded-xl p-3 mb-8 flex items-center gap-3">
            ...
            <p className="text-xs text-[#1D9E75]">
              You are on the <strong>Pro plan</strong> — full access to all lessons, live sessions and AI support.
            </p>
          </div>
        )}
            ...
            {courses.map((course) => {
              ...
              return (
                <Link key={course.id} href={`/course/${course.id}`}
                  ...
                  {nextLesson && <p className="text-[11px] text-[#5a6278] mb-3">Next: {nextLesson.title}</p>}
