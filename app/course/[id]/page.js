...
export default function CoursePage({ params }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (first) {
        setMessages([{ role: 'ai', text: `Hi ${session.user.name?.split(' ')[0]}! You are on "${first.title}". Ask me any doubt about this topic.` }])
      }
    })
  ...
  if (status === 'loading' || !course) {
    return (
      <div className="h-screen bg-[#0f1117] flex items-center justify-center">
        <p className="text-sm text-[#5a6278]">Loading course...</p>
      </div>
    )
  }
            ...
            return (
              <button key={lesson.id} onClick={() => accessible && selectLesson(lesson)}
                ...
                <div className="flex items-center gap-1.5 mb-0.5">
                  ...
                  {!accessible && <span className="text-[9px] text-[#534AB7]">🔒</span>}
            ...
            {hasAccess ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-4 p-6">
                ...
                <p className="text-xs text-[#8892a4] text-center max-w-xs">
                  You are on the {userPlan} plan. Upgrade to unlock all lessons and the full AI doubt agent.
                </p>
                <Link href="/#pricing" className="bg-[#534AB7] text-[#EEEDFE] text-xs font-medium px-5 py-2 rounded-lg hover:opacity-90 transition-opacity">
                  Upgrade plan
                </Link>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1.5">
                  ...
                  {progress[activeLesson?.id] && (
                    <span className="text-[10px] px-2 py-0.5 rounded bg-[#1e2a1e] text-[#1D9E75]">Completed</span>
                  )}
                </div>
              </div>
              <div className="flex gap-2">
                {hasAccess && !progress[activeLesson?.id] && (
                  <button onClick={markDone} className="text-xs bg-[#1D9E75] text-[#E1F5EE] px-3 py-1.5 rounded-lg font-medium hover:opacity-90 transition-opacity">
                    Mark done
                  </button>
                )}
                <button onClick={() => {
                ...
                }} className="text-xs bg-[#534AB7] text-[#EEEDFE] px-3 py-1.5 rounded-lg font-medium hover:opacity-90 transition-opacity">
                  Next →
                </button>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2.5">
            ...
            {aiLoading && (
              <div className="flex items-start">
                <div className="bg-[#0f1117] border border-[#2a2f3e] rounded-xl rounded-tl-sm px-3 py-2 text-[11px] text-[#5a6278]">Thinking...</div>
              </div>
            )}
            <div ref={bottomRef} />
          </div>
          ...
          <div className="p-3 border-t border-[#2a2f3e]">
            <div className="flex items-end gap-2 bg-[#0f1117] border border-[#2a2f3e] rounded-lg px-3 py-2">
              ...
              <button onClick={sendMessage} disabled={!input.trim() || aiLoading}
                className="w-7 h-7 rounded-md bg-[#534AB7] flex items-center justify-center hover:opacity-90 transition-opacity disabled:opacity-40">
