// lib/email.js  ← NEW FILE
// ─────────────────────────────────────────────────────────────────────────────
// Resend.com email client + all branded email templates for AST CodePath.
// To use: sign up at resend.com → get API key → add RESEND_API_KEY to Render.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function sendEmail({ to, subject, html }) {
  ...
  if (!apiKey) {
    // Dev mode: just log — don't crash if Resend not configured yet
    console.log(`[EMAIL] To: ${to} | Subject: ${subject}`)
    console.log('[EMAIL] Add RESEND_API_KEY to .env.local to send real emails')
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f1117;padding:40px 20px">
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0">
        ...
        <!-- Body card -->
        <tr><td style="background:#161b27;border:0.5px solid #2a2f3e;border-radius:12px;padding:32px">
          ${bodyContent}
        </td></tr>

        <!-- Footer -->
        <tr><td style="padding-top:20px;text-align:center">
          <p style="font-size:11px;color:#5a6278;margin:0">
            ${COMPANY.name} · <a href="${COMPANY.website}" style="color:#534AB7">${COMPANY.website}</a>
          </p>
          <p style="font-size:11px;color:#5a6278;margin:6px 0 0">
            This email was sent to you because you have an account on ${COMPANY.product}.
          </p>
        </td></tr>
...
// ── Template 1: Welcome email (sent on signup) ────────────────────────────────
export function welcomeEmail({ name }) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;font-weight:500;color:#e2e8f0;margin:0 0 8px">
      Welcome to ${COMPANY.product}, ${name.split(' ')[0]}!
    </h1>
    <p style="font-size:14px;color:#8892a4;line-height:1.7;margin:0 0 20px">
      Your account has been created. You are currently on the <strong style="color:#7f77dd">Free plan</strong> — 
      which gives you access to 2 free preview lessons and the AI doubt agent (5 questions/day).
    </p>
    ...
    <div style="text-align:center;margin:0 0 20px">
      <a href="${COMPANY.website}/dashboard" 
        style="background:#534AB7;color:#EEEDFE;font-size:14px;font-weight:500;padding:12px 28px;border-radius:8px;text-decoration:none;display:inline-block">
        Go to your dashboard →
      </a>
    </div>

    <div style="border-top:0.5px solid #2a2f3e;padding-top:16px;margin-top:8px">
      <p style="font-size:12px;color:#5a6278;margin:0">
        Need help? Reply to this email or contact us at 
        <a href="mailto:${COMPANY.email}" style="color:#534AB7">${COMPANY.email}</a> · +91 9866376367
      </p>
    </div>
  `)
...
export function passwordResetEmail({ name, resetUrl, expiresInMinutes = 30 }) {
  const html = emailWrapper(`
    ...
    <p style="font-size:14px;color:#8892a4;line-height:1.7;margin:0 0 20px">
      Hi ${name.split(' ')[0]}, we received a request to reset the password for your ${COMPANY.product} account.
    ...
    <div style="background:#0f1117;border:0.5px solid #2a2f3e;border-radius:8px;padding:14px;margin:0 0 20px">
      <p style="font-size:12px;color:#5a6278;margin:0">
        This link expires in <strong style="color:#8892a4">${expiresInMinutes} minutes</strong>. 
    ...
    <div style="border-top:0.5px solid #2a2f3e;padding-top:16px">
      <p style="font-size:12px;color:#5a6278;margin:0">
        ...
        <a href="${resetUrl}" style="color:#534AB7;word-break:break-all">${resetUrl}</a>
      </p>
    </div>
  `)
...
export function paymentConfirmationEmail({ name, plan, amount }) {
  const html = emailWrapper(`
    <h1 style="font-size:22px;font-weight:500;color:#e2e8f0;margin:0 0 8px">
      Payment confirmed — welcome to ${plan}!
    </h1>
    <p style="font-size:14px;color:#8892a4;line-height:1.7;margin:0 0 20px">
      Hi ${name.split(' ')[0]}, your subscription to ${COMPANY.product} ${plan} plan has been activated.
    </p>
    ...
    <div style="text-align:center;margin:0 0 20px">
      <a href="${COMPANY.website}/dashboard" 
        style="background:#1D9E75;color:#fff;font-size:14px;font-weight:500;padding:12px 28px;border-radius:8px;text-decoration:none;display:inline-block">
        Start learning now →
      </a>
    </div>
    ...
    <div style="border-top:0.5px solid #2a2f3e;padding-top:16px">
      <p style="font-size:12px;color:#5a6278;margin:0">
        ...
        Questions? Email <a href="mailto:${COMPANY.email}" style="color:#534AB7">${COMPANY.email}</a>
      </p>
    </div>
  `)
